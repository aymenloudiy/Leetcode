...
- If the total Cost is greater than the total Gas then there is no solution
- starting from the start of the array we calculate the how much our gas tank is holding in each position
- if the gas tank is less than what we have in the current position, it means we can't get to the next position from the current one, so we change out starting index to the next position and reset the gas tank
- because the problem guarantees a solution as long as the first condition in our approach is not met, we can be greedy.
## Solution
...
## Reflections
- Couldn't implement Kadane algorithm correctly and needed help from AI
- Could have done it alone, but I went over my time limit