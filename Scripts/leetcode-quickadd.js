...
async function promptForInput() {
  const input = await QuickAdd.quickAddApi.inputPrompt(
    "Enter LeetCode problem title slug or URL:"
  );
  if (!input) {
...
function setQuickAddVariables(problemData) {
  QuickAdd.variables = {
    ...problemData,
    fileName: `${problemData.id}. ${replaceIllegalFileNameCharactersInString(
      problemData.title
    )}`,
...
function formatExamples(markdown) {
  ...
  return markdown.replace(
    /Example \d+:([\s\S]*?)(?=Example \d+:|Constraints:|Follow-up:|$)/g,
    (match, content) => {
      const imageMatch = content.match(/!\[.*?\]\((.*?)\)/);
      const inputMatch = content.match(
        /Input:.*?([\s\S]*?)(?=<\/div>|Output|$)/i
      );
      const outputMatch = content.match(
        /Output:.*?([\s\S]*?)(?=<\/div>|Explanation|$)/i
      );
...
function formatConstraints(markdown) {
  const constraintsMatch = markdown.match(
    /Constraints:([\s\S]*?)(?=Follow-up:|$)/
  );
  if (constraintsMatch) {
